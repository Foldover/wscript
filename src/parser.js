...
function parse(input) {
    ...
    function parseLet() {
        ...
        if (input.peek().type === "identifier") {
            ...
            const defs = delimited("(", ")", ",", parseIdentifierDefinition);
            return {
                type: "call",
                func: {
                    type: "lambda",
        ...
        return {
            type: "let",
            vars: delimited("(", ")", ",", parseIdentifierDefinition),
            body: parseExpression(),
        }
    }
    ...
    function parseIdentifierDefinition() {
        let name = parseIdentifierName(), def;
        if (isOperator("=")) {
            input.next();
            def = parseExpression();
        }
